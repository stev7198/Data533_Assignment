...
class Skater(Player):
    """This class represents a skater and is inheriting from the player class. It allows the user to display basic and advanced stats for a selected skater"""
    def __init__(self, year):
        """Initializes the skater class with the year the user is interested in"""
        super().__init__(year)
        self.filename = f"skaters{self.year}.csv"

    def load_data(self):
        """Loads the skater data CSV"""
    ...
    def _show_basic_stats(self, player_name):
        """ Show basic stats for the player (Name, Team, Position, Goals, Points)"""
    ...
    def _show_advanced_stats(self, player_name):
        """ Show advanced stats for the player (Name, Team, Position, Corsit Percentage, Expected Goals, Fenwick Percentage)"""
        player_data = self._get_player_data(player_name)
        if player_data is not None:
